...
class PhpExcelComponent extends Component {
    ...
    public function openExcel($excelFileName) {
        ...
        $elementsChemin = pathinfo($excelFileName);
        ...
        if($elementsChemin['extension'] == 'xls'){
            $excel = $objPHPExcel->load($excelFileName);
        }else{
            $excel = $objReader->load($excelFileName);
        }
    ...
    public function downloadFile($excel, $filename) {
        ...
        header('Content-Disposition: attachment; filename=GDS-'.date('dmYHis').'.xls');
